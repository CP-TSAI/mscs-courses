...
class Robot:
    ...
    Attributes:
        ...
        max_distance: max distance robot can move.
    ...
    """
    def __init__(self, x: float = 0.0, y: float = 0.0, bearing: float = 0.0, max_distance: float = 1.0,
                 max_steering: float = PI/2, move_steering_noise: float = 0.0, move_distance_noise: float = 0.0,
    ...
    def set_noise(self, move_steering_noise: float, move_distance_noise: float, measure_distance_noise: float,
        ...
        """
        Set noise values for this robot.
    ...
    def move(self, steering: float, distance: float, noise: bool = False):
        """
        Move the robot.
    ...
    def measure_distance_and_bearing_to(self, point: Tuple, noise: bool = False):
        ...
        if noise:
            distance_sigma = self.measure_distance_noise * max(1.0, distance_to_point * 0.5 - 1.0)

            distance_to_point += random.gauss(0.0, distance_sigma)
